...
const props = defineProps({
    guru: Array, // menerima list guru yang belum jadi wali kelas
})

// ─── Label statis (tanpa kondisional workspace) ─────────────────────────────
const pageTitle = 'Add Class'
const heading = 'Add Class Data (Bulk)'
const subheading = 'Isi nama kelas di sebelah nama guru yang menjadi wali kelas. Tekan Enter untuk pindah ke baris berikutnya. Baris yang dikosongkan akan diabaikan.'
const colGuru = 'Homeroom Teacher (Walas)'
const colKelas = 'Class Name'
const kelasPlaceholder = 'Enter class name / unit...'
const cancelRoute = 'admin.kelas.index'
const saveBtn = 'Save All'

// satu baris per guru: { guru_id, nama_lengkap, kelas }
const rows = ref(
    props.guru.map(g => ({
        guru_id: g.id,
...
// ref ke setiap input kelas, untuk pindah fokus pakai Enter
const inputRefs = ref([])
...
const submit = () => {
    const items = rows.value
        .filter(r => r.kelas.trim().length > 0)
        .map(r => ({
            kelas: r.kelas.trim(),
            guru_id: r.guru_id,
        }))
            ...
            <div class="bg-white/70 dark:bg-gray-800/70 backdrop-blur-md rounded-2xl shadow-xl p-6 transition">
                ...
                <form @submit.prevent="submit" class="space-y-4">
                    ...
                    <div v-if="rows.length === 0" class="text-center py-6 text-gray-500 dark:text-gray-400">
                        Semua guru sudah menjadi wali kelas, atau belum ada data guru. Tambahkan guru baru jika
                        diperlukan.
                    </div>
                    ...
                    <template v-else>
                        ...
                        <div class="hidden sm:flex gap-3 px-1">
                            ...
                            <div class="w-1/2 text-sm font-medium text-gray-700 dark:text-gray-300">{{ colKelas }}</div>
                        </div>
                        ...
                        <div v-for="(row, index) in rows" :key="row.guru_id"
                            ...
                            <!-- Nama Kelas -->
                            <div class="sm:w-1/2">
                                <input :ref="el => setInputRef(el, index)" v-model="row.kelas" type="text"
                                    :placeholder="kelasPlaceholder" :disabled="form.processing"
