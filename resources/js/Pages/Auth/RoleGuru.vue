...
const preview = ref(null) // { nama_lengkap }

const form = useForm({ kode_guru: '' })

const cekKode = async () => {
    if (!form.kode_guru) return

    checking.value = true
    checkError.value = ''

    try {
        const { data } = await axios.get(route('role.guru.lookup'), {
            params: { kode_guru: form.kode_guru },
        })
        preview.value = data
        step.value = 'preview'
    } catch (e) {
...
const konfirmasi = () => {
    form.post(route('role.guru.store'), { preserveScroll: true })
}
    ...
    <div
        ...
        <div class="w-full max-w-md bg-white/80 backdrop-blur-xl rounded-3xl shadow-2xl border border-white/60 p-8">
            ...
            <div class="flex items-center gap-3 mb-1">
                ...
                <h1 class="text-xl font-bold text-gray-900">Verifikasi sebagai Guru</h1>
            </div>

            <!-- ══ STEP 1: Input kode ══ -->
            <template v-if="step === 'input'">
                <p class="text-sm text-gray-500 mb-6">
                    Masukkan kode guru yang diberikan sekolah untuk mengaitkan akun kamu.
                </p>

                <form @submit.prevent="cekKode" class="space-y-5">
                    <div>
                        <label class="block text-sm font-semibold text-gray-700 mb-1.5">Kode Guru</label>
                        <input v-model="form.kode_guru" type="text" placeholder="Contoh: GR-00123" autofocus class="w-full rounded-xl border px-4 py-2.5 text-sm bg-white text-gray-900 placeholder-gray-400
                    ...
                    <button type="submit" :disabled="!form.kode_guru || checking" class="w-full py-3 rounded-xl text-sm font-bold text-white bg-gradient-to-r from-blue-600 to-indigo-600
                               hover:from-blue-700 hover:to-indigo-700 disabled:opacity-50 transition">
                        {{ checking ? 'Memeriksa...' : 'Cek Kode' }}
                    </button>
                </form>
            </template>
            ...
            <template v-else>
                <p class="text-sm text-gray-500 mb-6">
                    Pastikan nama di bawah ini benar sebelum melanjutkan.
                </p>
                ...
                <div class="rounded-2xl border border-blue-100 bg-blue-50/60 p-5 mb-5 text-center">
                    ...
                    <p class="text-xs text-gray-400 mt-1">Kode: {{ form.kode_guru }}</p>
                </div>

                <p v-if="form.errors.kode_guru" class="mb-4 text-xs text-red-500 flex items-center gap-1">
                    <ExclamationTriangleIcon class="w-3.5 h-3.5 shrink-0" />
                    {{ form.errors.kode_guru }}
                </p>
                ...
                <div class="space-y-3">
                    <button type="button" @click="konfirmasi" :disabled="form.processing" class="w-full flex items-center justify-center gap-2 py-3 rounded-xl text-sm font-bold text-white
                        ...
                        {{ form.processing ? 'Menyimpan...' : 'Ya, Ini Saya - Lanjutkan' }}
                    </button>
