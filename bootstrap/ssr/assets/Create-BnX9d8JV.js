import { reactive, ref, computed, watch, withCtx, unref, createTextVNode, createVNode, withModifiers, withDirectives, openBlock, createBlock, Fragment, renderList, toDisplayString, vModelSelect, createCommentVNode, vModelText, useSSRContext } from "vue";
          ...
          if (_push2) {
            ...
            if (fileType.value === "file") {
              ...
              if (selectedFile.value) {
                ...
                _push2(`<span class="text-sm text-gray-500 dark:text-gray-400"${_scopeId}> Click to upload or drag &amp; drop </span>`);
              }
              _push2(`</div><input type="file" class="hidden" required accept=".jpg,.jpeg,.png,.pdf,.xls,.xlsx,.doc,.docx,.zip"${_scopeId}></label></div>`);
            } else {
              _push2(`<!---->`);
            }
            if (fileType.value === "link") {
              _push2(`<div class="space-y-1.5"${_scopeId}><label class="block text-sm font-medium text-gray-700 dark:text-gray-300"${_scopeId}> Enter Link <span class="font-normal text-gray-400"${_scopeId}>(YouTube, Google Drive, etc.)</span></label><input type="url"${ssrRenderAttr("value", form.link)} required maxlength="2048" class="w-full bg-gray-50 dark:bg-gray-800 border border-gray-200 dark:border-gray-700 rounded-xl px-3.5 py-2.5 text-sm text-gray-900 dark:text-white placeholder-gray-400 focus:outline-none focus:ring-2 focus:ring-blue-500 focus:border-transparent transition" placeholder="https://youtube.com/..."${_scopeId}></div>`);
            } else {
              _push2(`<!---->`);
            }
            if (fileType.value === "none") {
              _push2(`<p class="text-sm text-gray-400 dark:text-gray-500 italic"${_scopeId}> No attachment will be added to this submission. </p>`);
            } else {
              _push2(`<!---->`);
            }
            ...
            _push2(` ${ssrInterpolate(sending.value ? "Submitting..." : "Submit Assignment")}</button></div></form></div></div>`);
          } else {
            return [
              createVNode("div", { class: "min-h-screen sm:p-6 flex justify-center items-start" }, [
                createVNode("div", { class: "w-full" }, [
                  createVNode("div", { class: "mb-8" }, [
                    createVNode("h1", { class: "text-2xl font-bold text-gray-900 dark:text-white tracking-tight" }, "Submit Assignment"),
                    createVNode("p", { class: "text-sm text-gray-500 dark:text-gray-400 mt-1" }, "Fill in the details below to submit your assignment to your teacher.")
                  ]),
                  createVNode("form", {
                    ...
                    createVNode("div", { class: "bg-white dark:bg-gray-900 border border-gray-200 dark:border-gray-700 rounded-2xl p-5 shadow-sm space-y-4" }, [
                      ...
                      }, " No attachment will be added to this submission. ")) : createCommentVNode("", true)
                    ]),
                    createVNode("div", { class: "flex justify-end gap-3 pt-2 pb-8" }, [
                      ...
                      createVNode("button", {
                        ...
                        createTextVNode(" " + toDisplayString(sending.value ? "Submitting..." : "Submit Assignment"), 1)
                      ], 8, ["disabled"])
                    ])
                  ], 544)
                ])
...
_sfc_main.setup = (props, ctx) => {
  ...
  (ssrContext.modules || (ssrContext.modules = /* @__PURE__ */ new Set())).add("resources/js/Pages/Siswa/Assignment/Create.vue");
