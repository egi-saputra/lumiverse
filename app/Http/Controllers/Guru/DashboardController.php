...
class DashboardController extends Controller
{
    public function index()
    {
        ...
        return Inertia::render('Guru/Dashboard', [
            'auth' => ['user' => $user, 'role' => $user->role],
            'usersCount' => [
                'proktor' => User::where('role', 'proktor')->count(), 'guru' => User::where('role', 'guru')->count(),
                'siswa' => User::where('role', 'siswa')->count(), 'total' => User::count(),
            ],
            'isWalas' => Kelas::whereHas('guru', fn ($query) => $query->where('user_id', $user->id))->exists(),
            'insights' => [
